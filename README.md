...
```
Так как наше изображение трехмерное, то  ```tf.image.adjust_contrast``` настраивает контрастность независимо для каждого канала каждого изображения, а именно для каждого канала этот оператор вычисляет среднее значение пикселей изображения в канале, а затем настраивает каждый компонент x каждого пикселя на (x - mean) * contrast_factor + mean.  
