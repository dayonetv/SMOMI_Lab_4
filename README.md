...
Исходя из полученных результатов в качестве оптимальных параметров можно использовать: factor = 0.02, k = 0.3 (factor - параметр, определяющий диапазон из которого будет случайно выбран угол поворота, k - коэффициент наклона экспоненциальной кривой).
  + По сравнению с другими использовавшимися комбинациями:
    ...
    3. Небольшое значение потерь в конце обучения (значение потерь 0.2626, это хоть и не самое наименьшее значение, но все же лучше, чем без аугментации)  
  + По сравнению с политикой без аугментации данных:  
...
* Анализ полученных результатов  
Исходя из полученных результатов, можно сделать вывод, что только один из режимов заполнения, а именно reflect, привел к увеличению значения точности в конце обучения (точность выше на 0.08 процента), но в то же время, скорость сходимости алгоритма меньше на 5 эпох и потерь больше на 0.0099 по сравнению с поворотом изображения на случайный угол без использования режимов заполнения. Все остальные режимы заполнения (wrap, constant, nearest) привели к уменьшению значения точности и увеличению значения потерь.  
...
Для использования случайной части изображения были добавлены следующие функции:  
```
def process_data(image, label):
  return tf.image.random_crop(image, [224, 224, 3]), label
```
Для первоначального масштабирования изображения:
```
example['image'] = tf.image.resize(example['image'], tf.constant([250, 250]))  
```
```resize``` необходим для того, чтобы масштабировать наше изображение (224x224) до больших размеров (250x250). А затем к этому масштабированному изображению применяется ```random_crop``` для обрезки случайной части размером 224x224 из масштабированного изображения размером 250x250.  
Так как нейронная сеть EfficientNet-B0 требует подавать на вход изображение размером 224x224, то в данном случае параметры ```random_crop``` меняться не будут, а будут меняться параметры для ```resize``` (масштабирование), а именно высота (height) и ширина (width), которые будут у выходного изображения после применения этого слоя:  
...
Исходя из полученных результатов в качестве оптимальных параметров можно использовать: height = 250, width = 250 (высота (height) и ширина (width), которые будут у выходного изображения после применения слоя ```Resizing``` для масштабирования).
  + По сравнению с другими использовавшимися комбинациями:
    1. Алгоритму не удалось мойтись за весь период обучения (50 эпох), так же и у других комбинаций  
    2. Наибольшее значение метрики точности в конце обучения (89.09 процента)  
    3. Одно из наименьших значений потерь в конце обучения = 0.2624  
  + По сравнению с политикой без аугментации данных:  
    1. Не удалось увеличить скорость схождения алгоритма, по сравнению с политикой без аугментации данных (уменьшение скорости на 18 эпох)  
    2. Не удалось увеличить значение точности в конце обучения, по сравнению с политикой без аугментации данных (уменьшение точности на 0.08 процента)  
    3. Уменьшение значение потерь в конце обучения, по сравнению с политикой без аугментации данных (уменьшение потерь на 0.0029)  
***
Таким образом, техника использования случайной части изображения не привела к улучшению сходимости алгоритма обучения не по скорости, не по точности, только по значению потерь и то не намного (на 0.0029) при оптимально подобранных параметрах, в сравнении с политикой без аугментации данных. Более того, можно заметить, что качество ухудшается тем больше, чем больше масштабируется изображение перед тем как его обрезать.   
...
![изображение](https://user-images.githubusercontent.com/59259102/113316654-453ba600-9317-11eb-8c06-c5ab59182b03.png)  
...
![изображение](https://user-images.githubusercontent.com/59259102/113316723-54225880-9317-11eb-96f7-91ecdc96c470.png)  
...
Исходя из полученных результатов можно отметить следующее:  
  + Совместное применение техник аугментации данных (манипуляция с яркостью и контрастом, поворот изображения на случайный угол, использование случайной части изображения, добавление случайного шума) с оптимальными параметрами для каждой техники, позволило улучшить обучение нейронной сети на валидационном наборе данных, а именно:  
    1. Не увеличилась скорость схождения алгоритма, по сравнению с политикой без аугментации данных (уменьшение скорости на 19 эпох)  
    2. Увеличение точности в конце обучения (50 эпоха), по сравнению с политикой без аугментации данных (увеличение точности на 0.05 процента)  
    3. Уменьшение значения потерь в конце обучения (50 эпоха), по сравнению с политикой без аугментации данных (уменьшение значения потерь на 0.0097)  
  + Таким образом, совместное использование техник аугментации данных привело к улучшению сходимости алгоритма обучения по точности, и по значению потерь, но не по скорости обучения (алгоритм с техникой аугментации данных не сошелся за все время обучения 50 эпох) при оптимально подобранных параметрах для каждой техники, в сравнении с политикой без аугментации данных.  
